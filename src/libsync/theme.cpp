...
QIcon Theme::themeIcon(const QString &name, bool sysTray) const
{
    ...
    if (cached.isNull()) {
        ...
        const auto useSvg = shouldPreferSvg() && renderer.isValid();
...
QString Theme::themeImagePath(const QString &name, int size, bool sysTray) const
{
    ...
    const QString svgPath = QString(Theme::themePrefix) + QString::fromLatin1("%1/%2.svg").arg(flavor).arg(name);
    if (shouldPreferSvg() && QFile::exists(svgPath)) {
        return svgPath;
    }

    // branded client may have several sizes of the same icon; fall back to a sensible
    // default when no explicit size was requested (e.g. an icon's SVG is missing)
    const int effectiveSize = size > 0 ? size : 16;
    const QString pngPath = QString(Theme::themePrefix) + QString::fromLatin1("%1/%2-%3.png").arg(flavor).arg(name).arg(effectiveSize);
